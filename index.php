<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>



    <link rel="stylesheet" href="../agencia/bootstrap-5.2.3/css/bootstrap.min.css">
    <script defer src="../agencia/bootstrap-5.2.3/js/bootstrap.js"></script>

    <link rel="stylesheet" href="../agencia/css/style.css">

    <script type=text/js defer src="../js/main.js"></script>
</head>

<body class="body parallax">
<header>
        <div class="menu-container">
            <div class="menu">
                <a href="../../agencia/index.php"><div class="menu__logo"><img src="../../agencia/img/logo.png" alt="logo"></div></a>
                <div class="menu__unit">
                    <li><a href="../agencia/ua/about-page-ua.php">Про нас</a></li>
                    <li><a href="../agencia/ua/team-ua.php">Команда</a></li>
                    <li><a href="../agencia/ua/forwhom-ua.php">Для кого</a></li>
                    <li><a href="../agencia/ua/news-ua.php">Новини</a></li>
                    <li><a href="../agencia/ua/contacts-ua.php">Контакти</a></li>
                    <li><a target="_blank" href=" https://proukraine.com.ua/map/">Інвестиційний портал</a></li>
                </div>
                <!-- en/ua -->
                <div class="menu__lang">
                
                </div>
            </div>
        </div>
    </header>

    <section class="main">
        <section class="photo">


            <div id="carouselExampleControls" class="carousel slide" data-bs-ride="carousel">
                <div class="carousel_title">Одеська область</div>
                <div class="carousel-inner">
                    <div class="carousel-item active">
                        <img src="../agencia/img/ods-obl-1.jpg" class="d-block w-100" alt="...">
                    </div>
                    <div class="carousel-item">
                        <img src="../agencia/img/ods-obl-2.jpg" class="d-block w-100" alt="...">
                    </div>
                    <div class="carousel-item">
                        <img src="../agencia/img/odessa3.jpg" class="d-block w-100" alt="...">
                    </div>
                </div>
                <button class="carousel-control-prev" type="button" data-bs-target="#carouselExampleControls"
                    data-bs-slide="prev">
                    <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                    <span class="visually-hidden">Previous</span>
                </button>
                <button class="carousel-control-next" type="button" data-bs-target="#carouselExampleControls"
                    data-bs-slide="next">
                    <span class="carousel-control-next-icon" aria-hidden="true"></span>
                    <span class="visually-hidden">Next</span>
                </button>

            </div>

        </section>

        <section class="about-main">
            <div class="container">
                <div class="about-main__body">

                    <div class="about-main__body_title body-title">
                        <img class="h2_img" src="../agencia/img/bluearrow.svg" alt="">
                        <h2>Про Агенцію Розвитку Одеської області</h2>
                    </div>
                    <div class="about-main__body_text">
                        <p>Агенція регіонального розвитку Одеської області - це неприбуткова та не бюджетна установа, що
                            створена на засадах партнерства між державним, приватним та громадським секторами з метою
                            ефективної реалізації державної регіональної політики. <a class="about-main__body_text-a" href="../agencia/ua/about-page-ua.php">Детальніше...</a></p>
                    </div>


                </div>
            </div>
        </section>
        <section class="forwhom-main">
            <div class="container ">
                <div class="forwhom-main__body">
                    <div class="forwhom-main__body_business ">
                        <img src="../agencia/img/business.svg" alt="">
                        <h3>Бізнесу</h3>
                    </div>
                    <div class="forwhom-main__body_gromadam forwhom_item">
                        <img src="../agencia/img/gromadam.svg" alt="">
                        <h3>Громадам</h3>
                    </div>
                    <div class="forwhom-main__body_invest forwhom_item">
                        <img src="../agencia/img/invest.svg" alt="">
                        <h3>Інвесторам</h3>
                    </div>
                </div>
            </div>
        </section>
        <section class="odesaoblast">
            <div class="container">
                <div class="odesaoblast__body">
                    <div class="odesaoblast__body_title body-title">
                        <img class="h2_img" src="../agencia/img/bluearrow.svg" alt="">
                        <h2>Чому Одеська область:</h2>
                    </div>
                    <div class="odesaoblast__body_infographic">
                        <div class="infographic_title h3-title">
                            <img class="h3_img" src="../agencia/img/yellowarrow.svg" alt="">
                            <h3>Адміністративно-територіальний устрій </h3>
                        </div>
                        <div class="infographic_description"></div>
                        <div class="top-block">
                            <div class="top-block_item">
                                <div class="number">7</div>
                                <div class="description">районів</div>
                            </div>
                            <div class="top-block_item">
                                <div class="number">19</div>
                                <div class="description">міст</div>
                            </div>
                            <div class="top-block_item">
                                <div class="number">91</div>
                                <div class="description">ОТГ</div>
                            </div>
                        </div>




                    </div>
                    <div class="odesaoblast__body_infographic">
                        <div class="infographic_title h3-title">
                            <img class="h3_img" src="../agencia/img/yellowarrow.svg" alt="">
                            <h3>Кількість населення</h3>
                        </div>
                        <div class="infographic_description"></div>
                        <div class="top-block">
                            <div class="top-block_item">
                                <div class="number">2,4</div>
                                <div class="description">млн. осіб</div>
                            </div>
                            <div class="top-block_item">
                                <div class="number">56%</div>
                                <div class="description">працездатних</div>
                            </div>
                            <div class="top-block_item">
                                <div class="number">91</div>
                                <div class="description">ОТГ</div>
                            </div>
                        </div>
                    </div>
                    <div class="odesaoblast__body_infographic">
                        <div class="infographic_title h3-title">
                            <img class="h3_img" src="../agencia/img/yellowarrow.svg" alt="">
                            <h3>Вигідне економіко-географічне розташування</h3>
                        </div>
                        <div class="infographic_description"> Межування з 3-ма областями з населенням більше ніж 3,5
                            млн. чоловік.</div>
                        <div class="top-block">
                            <div class="top-block_item">
                                <div class="number">1,6<div class="number-min"> млн</div></div>
                                <div class="description">Вінницька обл.</div>
                            </div>
                            <div class="top-block_item">
                                <div class="number">933 <div class="number-min">тис</div></div>
                                <div class="description">Кіровоградська обл.</div>
                            </div>
                            <div class="top-block_item">
                                <div class="number">1,2<div class="number-min"> млн</div></div>
                                <div class="description">Миколаївська обл.</div>
                            </div>
                        </div>
                        <div class="infographic_description">Межування з 2-ма країнами з населенням більше ніж 20 млн.
                            чоловік.</div>
                        <div class="top-block">
                            <div class="top-block_item">
                                <div class="number">2,6 <div class="number-min">млн</div></div>
                                <div class="description">Республіка Молдова</div>
                            </div>
                            <div class="top-block_item">
                                <div class="number">19,4 <div class="number-min">млн</div></div>
                                <div class="description">Румунія</div>
                            </div>

                        </div>

                    </div>
                    <div class="odesaoblast__body_infographic">
                        <div class="infographic_title h3-title">
                            <img class="h3_img" src="../agencia/img/yellowarrow.svg" alt="">
                            <h3>Представлені всі види транспорту</h3>
                        </div>
                        <div class="infographic_description"></div>
                        <div class="top-block">
                            <div class="top-block_item">
                                <div class="number">5</div>
                                <div class="description">міжнародних транспортних коридори</div>
                            </div>
                            <div class="top-block_item">
                                <div class="number">2</div>
                                <div class="description">коридори транс’європейської транспортної мережі</div>
                            </div>
                            <div class="top-block_item">
                                <div class="number">7</div>
                                <div class="description">морських портів</div>
                            </div>
                        </div>
                    </div>


                    <div class="odesaoblast__body_infographic">
                        <div class="infographic_title h3-title">
                            <img class="h3_img" src="../agencia/img/yellowarrow.svg" alt="">
                            <h3>Сприятливі умови для сільського господарства</h3>
                        </div>
                        <div class="infographic_description"></div>
                        <div class="top-block">
                            <div class="top-block_item">
                                <div class="number">2,5 <div class="number-min">млн. га</div></div>
                                <div class="description">Площа сільськогосподарських угідь</div>
                            </div>
                            <div class="top-block_item">
                                <div class="number">чорнозем</div>
                                <div class="description">грунт</div>
                            </div>

                        </div>
                        <div class="infographic_description"> Площа, що перевищює земельні ресурси низки країн ЄС,
                            наприклад: Австрії, Нідерландів та ін. Клімат - помірно-континетальний, рівнинна місцевість,
                            грунт - чернозем.</div>

                    </div>

                    <div class="odesaoblast__body_infographic">
                        <div class="infographic_title h3-title">
                            <img class="h3_img" src="../agencia/img/yellowarrow.svg" alt="">
                            <h3>Корисні копалини</h3>
                        </div>
                        <div class="infographic_description"></div>
                        <div class="top-block">
                            <div class="top-block_item">
                                <div class="number">145</div>
                                <div class="description">родовищ корисних копалин</div>
                            </div>
                            <div class="top-block_item">
                                <div class="number">80%</div>
                                <div class="description">з яких виробляють буд. матеріали</div>
                            </div>

                        </div>
                    </div>
                    <div class="odesaoblast__body_infographic">
                        <div class="infographic_title h3-title">
                            <img class="h3_img" src="../agencia/img/yellowarrow.svg" alt="">
                            <h3>
                                Трудовий потенціал
                            </h3>
                        </div>
                        <div class="infographic_description"></div>
                        <div class="top-block">
                            <div class="top-block_item">
                                <div class="number">36</div>
                                <div class="description">закладів професійно-технічної освіти</div>
                            </div>
                            <div class="top-block_item">
                                <div class="number">> 40</div>
                                <div class="description">вищих навчальних закладів</div>
                            </div>
                            <div class="top-block_item">
                                <div class="number">15</div>
                                <div class="description">науково-дослідницьких інститутів</div>
                            </div>

                        </div>
                    </div>
                    <div class="odesaoblast__body_infographic">
                        <div class="infographic_title h3-title">
                            <img class="h3_img" src="../agencia/img/yellowarrow.svg" alt="">
                            <h3>
                                Бізнес середовище
                            </h3>
                        </div>
                        <div class="infographic_description"></div>
                        <div class="top-block">
                            <div class="top-block_item">
                                <div class="number">> 130 <div class="number-min">тис.</div></div>
                                <div class="description">закладів професійно-технічної освіти</div>
                            </div>
                            <div class="top-block_item">
                                <div class="number">> 20 </div>
                                <div class="description">вищих навчальних закладів</div>
                            </div>


                        </div>
                        <div class="top-block">

                            <div class="top-block">
                                <div class="top-block_item">
                                    <div class="number">2,2 <div class="number-min">млр.дол</div></div>
                                    <div class="description">Експорт 2020 р.</div>
                                </div>
                                <div class="top-block_item">
                                    <div class="number">2,1<div class="number-min"> млр.дол.</div></div>
                                    <div class="description">Імпорт 2020 р.</div>
                                </div>


                            </div>





                        </div>
                    </div>
                    <div class="odesaoblast__body_infographic">
                        <div class="infographic_title h3-title">
                            <img class="h3_img" src="../agencia/img/yellowarrow.svg" alt="">
                            <h3>
                                Туристична привабливість
                            </h3>
                        </div>
                        <div class="infographic_description">Сприятливі природно-кліматичні умови та
                            курортно-рекреаційні ресурси
                        </div>
                        <div class="top-block">
                            <div class="top-block_item">
                                <div class="number"> > 200</div>
                                <div class="description">закладів розміщення</div>
                            </div>
                            <div class="top-block_item">
                                <div class="number">396</div>
                                <div class="description">закладів харчування</div>
                            </div>
                            <div class="top-block_item">
                                <div class="number">320</div>
                                <div class="description">санаторно-курортних та оздоровчих закладів </div>
                            </div>

                        </div>
                        <div class="top-block">
                            <div class="top-block_item">
                                <div class="number">150,8<div class="number-min"> тис. га</div class="number-min"></div>
                                <div class="description">природно-заповідних територій</div>
                            </div>
                            <div class="top-block_item">
                                <div class="number">> 50</div>
                                <div class="description"> туристичних маршрутів</div>
                            </div>
                            <div class="top-block_item">
                                <div class="number">6 <div class="number-min">млн. осіб</div></div>
                                <div class="description">Rількість туристів у 2020 р.</div>
                            </div>

                        </div>
                    </div>
                </div>
            </div>
        </section>


    </section>
    <?php
    include('ua/footer-ua.php');
    ?>
    
</body>

</html>